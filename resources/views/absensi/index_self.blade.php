...
@section('subtitle','Bulan '.Calendar::indonesiaMonth(empty(Request::get('bulan'))?date('m'):Request::get('bulan'),true))
@section('content')
...
<div class="modal fade" id="detail-absen" tabindex="-1" role="dialog" aria-labelledby="absenLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    ...
    <div class="row">
        ...
        <div class="col-12">
         {!! Calendar::absenGenerate((empty(Request::get('bulan'))? date('m'):Request::get('bulan')),(empty(Request::get('tahun'))? date('Y'):Request::get('tahun')),auth()->user()->karyawan_id)!!}
  ...
  axios.post('{{route("absen.keluar.request")}}', formData)
    ...
    window.tableSelf = $('#absenTable').DataTable({
                  ...
                  "initComplete": function(settings, json) {
                    ...
                    ajax: "{{ route('absen.datatables.self')}}?tahun={{Request::get('tahun')}}&bulan={{Request::get('bulan')}}",
                    columns: [{
                            data: 'foto',
                            name: 'foto'
                        },{
      ...
      {
        let formData = new FormData();
        axios.post('{{route("absen.getDataAbsenStatSelf")}}?tahun={{Request::get('tahun')}}&bulan={{Request::get('bulan')}}', formData)
                .then(function (response) {
      ...
      function details(id){
        ...
        formData.append('id', id);
        axios.post('{{route("absensi.showAbsenSelf")}}', formData)
                .then(function (response) {
